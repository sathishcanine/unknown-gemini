...
def call_gemini_generation(topic, batch_num, facts, exclusion_texts, api_key):
    """Calls Gemini to generate practice questions based on facts."""
    # Keep up to 80 facts to stay within context size guidelines
    sampled_facts = facts
    if len(facts) > 80:
    ...
    prompt = f"""
You are a senior exam compiler for the TNPSC Group I/II Civil Services specializing in General Science & Chemistry.
...
4. Target formats:
   ...
   - Standard multiple-choice questions with plausible distractors. Distractors must be highly plausible, using adjacent chemical compounds, formulas, discoveries, or reactions to make them challenging.
5. Advanced Formats:
   - Paragraph-Based Inference Questions (Min 3 per call): Feature a data-rich premise. Options must test logical deduction rather than rote memory.
   - Contextual Connect Questions (Min 2 per call): Hook core chemical substances, discoveries, or industrial manufacturing processes to their objectives, properties, or daily life applications.
6. JSON Output Format:
   Output a raw JSON array of objects. Do not wrap in markdown or include introductions. You MUST strictly escape all double quotes inside JSON string values as \" to prevent syntax errors. Each object must have these exact keys:
...
def main():
    parser = argparse.ArgumentParser(description="Chemistry Practice Question Generator")
    ...
    facts_path = "/Users/sathishkumar/Pictures/Ai-Demos/Q-Gemini/Chemistry/chemistry_facts.json"
    ...
    print(f"Total questions in Chemistry database: {len(all_db_qs)}")
