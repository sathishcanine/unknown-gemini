# Indian History MCQ Batch Generation Guide

This guide is designed for **the User** and **future AI Agents** to ensure consistent, duplicate-free question generation for any topic in the **Indian History** syllabus.

---

## 1. What to Ask the AI (User Prompts)

When you return and want to generate new batches for a History topic, copy-paste one of these prompts:

### Option A: Generate the Next Batch for an Existing Topic
> "I want to generate **Batch [X]** for the topic **[Topic Name]** in Indian History. 
> 1. Read `Indian-History/HISTORY_BATCH_GENERATION_GUIDE.md` for the history generation rules and guardrails.
> 2. Load `Indian-History/history_questions_db.json` and gather all existing questions (both PYQs and practice batches) under the topic '[Topic Name]' to use as exclusions.
> 3. Use the facts in the respective topic key in `Indian-History/history_facts.json`.
> 4. Generate 34 questions, prune to 30 (15 Medium, 15 Hard), shuffle, and append to the database."

### Option B: Start a New History Topic from Scratch
> "I want to start a new Indian History topic: **[Topic Name]**.
> 1. Locate the study files in the `Data/Indian-History-Data/` directory.
> 2. Search for the relevant chapters/pages.
> 3. Extract the text and run a page-by-page fact extraction to save to `Indian-History/history_facts.json`.
> 4. Transcribe this topic's PYQs from `INDIAN HISTORY PYQ PDF 2020 - 2025.pdf` and save them to `Indian-History/history_questions_db.json` first.
...
Every AI agent must follow these strict guidelines for the Indian History subject:

### Rule 1: Source of Truth
* Only learn facts from the official study material PDFs in the `Data/Indian-History-Data/` directory (Gurunath, Suresh History). **Do not learn facts from the PYQs** (only use PYQs to learn question patterns, wording, and style).

### Rule 2: Complete Pool Analysis & Zero Duplicates
* **No Fact Slicing**: For every batch (including Batch 1, Batch 2, and onward), the generator script **must** load and analyze the **entire pool of facts** for that topic in `history_facts.json`, rather than slicing them into subsets.
* **Strict Question Exclusion**: To guarantee zero duplication, the script loads all existing questions from `history_questions_db.json` for that topic and passes their English text as a strict exclusion list. The API will choose from the entire fact pool but must craft entirely new questions that do not duplicate the existing ones.
...
2. **Contextual Connect Questions (Min 4)**: Connect historical systems (e.g. Mansabdari, Iqta, local assemblies) or monuments/literature to their stated administrative, cultural, or social objectives.
...
If the User asks to generate a practice batch for an Indian History topic (whether a new one or an existing one), you **must not** generate any questions until you perform the following verification:

1. **Verify Facts File**: Check if there is an extracted facts JSON entry for the topic in `Indian-History/history_facts.json`.
2. **Verify PYQs**: Check if the topic has its Previous Year Questions (PYQs) transcribed and saved in `Indian-History/history_questions_db.json` under `"type": "pyq"`.
...
## 4. Database Location
* **Database**: `Indian-History/history_questions_db.json` (in the workspace folder).
* **Subject Key**: `"History"`
