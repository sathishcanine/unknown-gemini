...
DB_PATH = os.path.join(BASE_DIR, "Policy", "policy_questions_db.json")
FACTS_PATH = os.path.join(BASE_DIR, "Policy", "policy_facts.json")
GUIDE_PATH = os.path.join(BASE_DIR, "Policy", "POLICY_GENERATION_GUIDE.md")
...
def call_gemini_generation(topic, batch, facts, exclusion_texts, api_key):
    """Builds a structured prompt for practice question generation based on the sliced facts."""
    facts_str = "\n".join([f"- Fact: {f['fact_en']} | Source: {f.get('source', '')}" for f in facts])
    exclusions_str = "\n".join([f"- {t}" for t in exclusion_texts[:60]]) # Limit exclusion list size

    prompt = f"""
You are an expert TNPSC question generator.
Generate practice questions based on the Tamil Nadu Government Policy Note for the topic "{topic}".
This is Batch {batch}.
...
4. Direct MCQ Fact-Checks (20% - 30%): Single choice questions on budgets, stats, etc.

--- RULES FOR DISTRACTORS AND EXPLANATIONS ---
- All options (A, B, C, D) must be highly plausible. Use close budget numbers, adjacent years, or related departments.
...
def main():
    parser = argparse.ArgumentParser(description="TNPSC Policy Note Batch Generator")
    parser.add_argument("--topic", required=True, help="Canonical department name (e.g. 'Agriculture')")
    ...
    print(f"\nSUCCESS: Added 30 unique questions for topic '{args.topic}' (Batch {args.batch}) to database!")
    print(f"Total questions in Policy database: {len(all_db_qs)}")
