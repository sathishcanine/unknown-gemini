# TVK-Government Policies Generation Guide

This guide codifies the rules, question patterns, and distractor constraints for generating TVK-Government Policies practice batches for the TNPSC Group 1 & 2 exams.

Source material covers TVK Leaders notes and TVK Policies & Schemes parts (scanned PDFs under `Data/TVK-Government-Data/`).
...
2. **Contextual Connect Questions (Max 2)**: Connect a scheme, leader promise, budget allocation, or target to its stated policy objective, beneficiary group, or implementation timeline mentioned in the facts.

---

## 2. Core Question Patterns

Every TVK practice batch must contain a balanced mix of these standard TNPSC question formats:

### Pattern A: Statement-Evaluation (Target: 30% - 40% of Batch)
Evaluates multiple dimensions of a single TVK policy, scheme, leader commitment, or allocation.
* **Structure**: Present 2 to 4 numbered statements (1, 2, 3, 4). For e.g., detailing budgets, eligible categories, or target districts.
...
Matches lists of related entities (e.g., Schemes → Benefits, Leaders → Portfolios/Promises, Targets → Budgets).
...
Tests singular, crucial statistics (e.g., scheme names, amounts, launch dates, leader-related facts).

---

## 3. Plausible Distractors & Formatting
* **Plausible Distractors**: All incorrect options must use adjacent years, related scheme names, realistic budget allocations (e.g. ₹5,000 cr instead of ₹5,200 cr), or close statistical percentages to test precision.
...
| TVK Policies & Schemes Part 2 | `TVK_govt_Policy_Scheme_part_2_usable.pdf` | `0..19` |
| TVK Policies & Schemes Part 3 | `Tvk_govt_policy_Scheme_part_3_usable.pdf` | `0..48` |

Subject id in questions DB: `TVK`.
