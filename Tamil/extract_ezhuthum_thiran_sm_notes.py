#!/usr/bin/env python3
"""
Extract Ezhuthum Thiran / Marabu Tamil (Unit 3) rules + examples from SM Tamil Full Book via Gemini OCR.

Writes into Tamil/ezhuthum_thiran_notes.json → topics[<id>].rules / .examples
Preserves existing pyq_samples.

Usage:
  python3 Tamil/extract_sollagarathi_sm_notes.py --topic sorkalai_ozhungu_sotrodar
...
SOURCE_MAP_PATH = os.path.join(BASE_DIR, "Tamil", "ezhuthum_thiran_source_map.json")
...
def call_gemini_sm_page(topic_name, pdf_page, printed_page, img_base64, api_key):
    prompt = f"""
You are extracting TNPSC General Tamil Unit 2 (எழுதும் திறன் / மரபுத் தமிழ் (Writing skill / Traditional Tamil)) NOTES from a scanned textbook page.
...
1) RULES — definitions, how to pick sentence order, clause types, verb voice, number agreement, thinai/paal, marabu lists, punctuation, tip boxes.
2) EXAMPLES — word pairs or lists shown on the page (word → antonym/meaning/equivalent, groups, orders).
...
- For examples (antonyms / meaning pairs):
  ...
  - "kind" = "reorder" | "define" | "classify" | "transform" | "agreement" | "marabu" | "punctuate" | "example" | "other"
...
- Extract worked reorder examples (jumbled → correct), definitions, classification lists, and punctuation placement rules with examples.
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Unit 3 Ezhuthum Thiran SM notes (rules+examples) via Gemini OCR"
    )
    parser.add_argument("--topic", help="topic id, e.g. sorkalai_ozhungu_sotrodar")
