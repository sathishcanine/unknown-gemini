#!/usr/bin/env python3
"""
Extract Sollagarathi (Unit 2) rules + examples from SM Tamil Full Book via Gemini OCR.

Writes into Tamil/sollagarathi_notes.json → topics[<id>].rules / .examples
Preserves existing pyq_samples.

Usage:
  python3 Tamil/extract_sollagarathi_sm_notes.py --topic ethirchol_eduthelzhuthal
...
SOURCE_MAP_PATH = os.path.join(BASE_DIR, "Tamil", "sollagarathi_source_map.json")
...
def call_gemini_sm_page(topic_name, pdf_page, printed_page, img_base64, api_key):
    prompt = f"""
You are extracting TNPSC General Tamil Unit 2 (சொல்லகராதி / Vocabulary) NOTES from a scanned textbook page.
...
1) RULES — definitions, how to pick antonyms / meanings / odd-one-out / alphabetical order, tip boxes.
...
- For examples (antonyms / meaning pairs):
  ...
  - "kind" = "antonym" | "meaning" | "synonym" | "order" | "odd_one" | "fill" | "other"
...
- If the page is mostly tables of antonym/meaning pairs, extract MANY examples; rules may be few.
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Sollagarathi SM notes (rules+examples) via Gemini OCR"
    )
    parser.add_argument("--topic", help="topic id, e.g. ethirchol_eduthelzhuthal")
