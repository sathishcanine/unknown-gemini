#!/usr/bin/env python3
"""
Generate TNPSC Kalaichorkal / Technical Terms (Unit 4) practice batches from kalaichorkal_notes.json.

Usage:
  python3 Tamil/generate_kalaichorkal_questions.py --topic sorkalai_ozhungu_sotrodar --batch 1
"""
...
DB_PATH = os.path.join(BASE_DIR, "Tamil", "kalaichorkal_questions_db.json")
...
def preferred_shapes(topic_id: str, topic_ta: str, topic_en: str) -> str:
   ...
   - English technical term → choose the correct Tamil equivalent (from notes)
   - Tamil கலைச்சொல் → choose the English / meaning (from notes)
   - Spot the WRONG EN↔TA pair among close distractors
   - Optional: domain tag (கல்வி / மருத்துவம் / சட்டம்…) if in notes
6. Distractors = nearby SM terms from SAME domain — NOT invented translations.
   Use ONLY notes. Pure Tamil for Tamil options. NO SM branding / Latin rule codes."""
...
def call_gemini(topic_id, topic_ta, topic_en, batch_num, rules, examples, exclusion_texts, api_key):
    ...
    prompt = f"""
You are a senior TNPSC General Tamil (பொதுத் தமிழ்) exam compiler for Unit 4 கலைச் சொற்கள்.
...
def main():
    parser = argparse.ArgumentParser(description="Kalaichorkal practice question generator")
