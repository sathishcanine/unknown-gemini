...
DB_PATH = os.path.join(ROOT_DIR, "Central-Gov-Schemes", "cgs_questions_db.json")

SUBJECT_ID = "CGS"
SUBJECT_NAME = "Central Government Schemes"
SUBJECT_NAME_TA = "மத்திய அரசுத் திட்டங்கள்"
SUBJECT_ICON = "🇮🇳"
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        print(f"Error: CGS question database not found at {DB_PATH}")
        return
    ...
    print(f"Loaded {len(questions)} CGS questions from JSON. Importing to DB...")
        ...
        if isinstance(raw_options, dict):
            ...
            for idx, opt in enumerate(raw_options):
                if isinstance(opt, dict):
                    ...
                    # Strip leading "A) " / "E) " prefixes if present
                    for prefix in ("A)", "B)", "C)", "D)", "E)", "A.", "B.", "C.", "D.", "E."):
                        if text_en.startswith(prefix):
                            text_en = text_en[len(prefix):].strip()
                            break
