...
DB_PATH = os.path.join(ROOT_DIR, "Indian-History", "history_questions_db.json")
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        print(f"Error: History question database not found at {DB_PATH}")
        return

    conn = psycopg2.connect(db_url)
    cursor = conn.cursor()
    
    # 1. Ensure subject 'History' exists
    cursor.execute("SELECT id FROM subjects WHERE id = %s;", ("History",))
    if not cursor.fetchone():
        cursor.execute("INSERT INTO subjects (id, name, icon) VALUES (%s, %s, %s);", ("History", "Indian History", "📜"))
        print("Created History subject metadata.")
    ...
    def get_or_create_topic(topic_name):
        ...
        cursor.execute("SELECT id FROM topics WHERE subject_id = %s AND name = %s;", ("History", key))
        row = cursor.fetchone()
        if row:
            topic_cache[key] = row[0]
            return row[0]
            
        cursor.execute("INSERT INTO topics (subject_id, name) VALUES (%s, %s) RETURNING id;", ("History", key))
        topic_id = cursor.fetchone()[0]
        topic_cache[key] = topic_id
        print(f"Created topic: '{key}'")
        return topic_id

    # 2. Read the transcribed history questions JSON
    with open(DB_PATH, "r", encoding="utf-8") as f:
        questions = json.load(f)
        
    print(f"Loaded {len(questions)} history questions from JSON. Importing to DB...")
    ...
    for q in questions:
        ...
        """, ("History", topic_id, q_en, q_ta, correct_opt, exp, exp_ta, diff, q_type, batch, exam, fact))
