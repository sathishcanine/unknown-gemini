#!/usr/bin/env python3
"""Import Tamil Ilakkanam practice questions into Postgres."""
...
DB_PATH = os.path.join(ROOT_DIR, "Tamil", "ilakkanam_questions_db.json")
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        print(f"Error: Ilakkanam question database not found at {DB_PATH}")
        return
    ...
    print(f"Loaded {len(questions)} Ilakkanam questions from JSON. Importing to DB...")
    ...
    for q in questions:
        ...
        # Dedup on Tamil stem first (Ilakkanam is Tamil-first), fall back to EN
        cursor.execute(
            """
    ...
    cursor.execute(
        ...
        FROM questions WHERE subject_id = %s;
        """,
        (SUBJECT_ID,),
    )
    ...
    cursor.execute(
        """
        SELECT batch, COUNT(*) FROM questions
        WHERE subject_id = %s GROUP BY batch ORDER BY batch;
        """,
        (SUBJECT_ID,),
    )
