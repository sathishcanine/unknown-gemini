#!/usr/bin/env python3
"""Import Tamil Sollagarathi (Unit 2) practice questions into Postgres."""
...
DB_PATH = os.path.join(ROOT_DIR, "Tamil", "sollagarathi_questions_db.json")
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        print(f"Error: Sollagarathi question database not found at {DB_PATH}")
        return
    ...
    print(f"Loaded {len(questions)} Sollagarathi questions from JSON. Importing to DB...")
    ...
    topic_ta = "எதிர்ச்சொல்லை எடுத்தெழுதுதல்"
    cursor.execute(
        """
